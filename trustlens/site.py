...
T = {
    "zh": {
        ...
        "method_mcp": "<b>评测方法</b>：隔离沙箱 + 零凭证开箱即用（90s 握手超时）实测；每个工具的真实调用参数由 <b>DeepSeek-V4 Flash</b> 按 schema 生成并经安全护栏清洗（只读已有结果，评测步骤不持有任何密钥）；模型工具调用维度同样由 <b>DeepSeek-V4 Flash</b> 真实调用评测。安全维度为<b>静态文本模式扫描</b>（提示注入/数据外发/硬编码凭证），只检出明显特征，<b>无法检出运行时外发等隐蔽行为——\"安全满分\"≠\"已证实安全\"</b>。全部数据以 JSON 公开在仓库，git 历史即审计日志。",
