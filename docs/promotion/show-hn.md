...
**Title:**
Show HN: I benchmarked 119 real MCP servers — 55% are unusable out of the box
...
TrustLens is an open-source evaluation engine that actually runs each server in a sandbox: protocol handshake, static security scan (prompt injection / exfiltration / hardcoded creds), real probe calls with latency and failure tracking, multi-model compatibility. It outputs a 0–100 trust score with fully public evidence (JSON data committed to the repo; git history is the audit log).

I evaluated 119 real npm/PyPI MCP servers in a zero-credential environment. Result: **only 9 grade A (8%); 66 (55%) are F** — they never complete a handshake with the standard `npx -y <pkg>` start and no config. Failure modes: startup hang (55), crash on missing API key (7), SDK compat crash (3). A few official Python servers crash on import because the mcp SDK renamed McpError → MCPError.
