...
- **MCP 榜**：实测 118 个真实 MCP 服务器，**55%（65 个）开箱即用连协议握手都完不成**；只有 1 个 A 级（ifconfig-mcp 100 分）
- **Skills 榜**：**110 个真实 Agent Skills**（官方 + 社区）三维评测（结构/安全/LLM 可执行性），A:19 B:81 C:10
- **真实模型背书**：模型工具调用分由 **DeepSeek-V4 Flash 真实调用**得出——"同一个工具，模型到底能不能用起来"没人发布过
- 失败原因可讲故事：运行时崩溃 30 / 缺凭证崩 15 / 包解析 11 / 真·挂起 9
- 反例也成立：官方 filesystem 只有 C 级、网红 desktop-commander 只有 D 级——**"热门≠靠谱"**
